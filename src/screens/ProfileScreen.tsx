...
import { User, Shield, Moon, Sun, Download, Upload, RotateCcw, Smartphone, Check, AlertTriangle, X } from 'lucide-react';
...
export const ProfileScreen: React.FC<ProfileScreenProps> = ({
  ...
  const getRankTitle = (xp: number) => {
    ...
    if (xp >= 200) return { title: 'Desenvolvedor Junior 💻', color: 'text-orange-400' };
    return { title: 'Dev Aprendiz 🐣', color: 'text-white/60' };
  };
  ...
  return (
    <div className="pb-24 pt-4 px-4 max-w-md md:max-w-2xl mx-auto space-y-5">
      {/* Toast Notification */}
      {toastMessage && (
        <div className="fixed top-16 left-1/2 -translate-x-1/2 z-50 bg-orange-500 text-black font-extrabold text-xs px-4 py-2 rounded-full shadow-2xl flex items-center gap-2 animate-bounce">
      ...
      {/* Header Profile Card */}
      <div className="p-6 rounded-3xl bg-[#1A1A1C] border border-white/10 text-center space-y-3 relative overflow-hidden shadow-xl">
          ...
          {isEditingName ? (
            <div className="flex items-center justify-center gap-2 max-w-xs mx-auto">
              ...
              <button
                ...
                className="px-3.5 py-1.5 bg-orange-500 text-black font-extrabold text-xs rounded-xl uppercase tracking-wider"
              >
                Salvar
              </button>
            </div>
            ...
            <div className="flex items-center justify-center gap-2">
              ...
              <button
                ...
                className="text-[11px] text-orange-400 font-bold hover:underline uppercase tracking-wider"
              >
                Editar
              </button>
            </div>
      ...
      <div className="p-4 rounded-2xl bg-[var(--bg-card)] border border-[var(--border-subtle)] space-y-3 shadow-md">
      ...
      <div className="p-4 rounded-2xl bg-[#1A1A1C] border border-white/10 space-y-3 shadow-md">
        ...
        <div className="grid grid-cols-2 gap-2">
          <button
            ...
            className="flex items-center justify-center gap-2 p-3 bg-black/40 hover:bg-black/80 border border-white/10 hover:border-white/20 rounded-xl text-xs font-bold text-white transition-colors min-h-[44px] uppercase tracking-wider"
          >
        ...
        <button
          onClick={onExportData}
          className="w-full py-3.5 border border-white/20 font-bold text-xs uppercase tracking-widest text-white hover:bg-white hover:text-black transition-colors rounded-xl shadow-sm"
        >
        ...
        <button
          ...
          className="w-full mt-2 flex items-center justify-center gap-2 p-3 bg-red-950/30 hover:bg-red-900/40 border border-red-500/30 rounded-xl text-xs font-bold text-red-400 transition-colors min-h-[44px] uppercase tracking-wider"
        >
      ...
      {showImportModal && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#1A1A1C] border border-white/10 rounded-2xl p-5 max-w-sm w-full space-y-3">
            ...
            <button
              ...
              className="w-full py-2.5 bg-orange-500 text-black font-extrabold text-xs rounded-xl uppercase tracking-wider"
            >
              Restaurar Dados
            </button>
          </div>
      ...
      {showResetModal && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#1A1A1C] border border-white/10 rounded-2xl p-5 max-w-sm w-full space-y-3 text-center">
            ...
            <div className="flex gap-2 pt-2">
              <button
                ...
                className="flex-1 py-2.5 bg-white/10 text-white font-bold text-xs rounded-xl uppercase tracking-wider"
              >
                Cancelar
              </button>
              <button
                ...
                className="flex-1 py-2.5 bg-red-600 text-white font-bold text-xs rounded-xl uppercase tracking-wider"
              >
                Resetar Tudo
              </button>
            </div>
      ...
      {showMobileModal && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#1A1A1C] border border-white/10 rounded-2xl p-5 max-w-md w-full space-y-3 max-h-[80vh] overflow-y-auto">
