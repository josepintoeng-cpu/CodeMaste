  ...
  return (
    <div className="relative pb-28 pt-4 px-3.5 sm:px-6 md:px-8 max-w-7xl mx-auto space-y-6">
      ...
      <motion.div variants={fadeInUp} initial="initial" animate="animate">
        <div className="text-[10px] sm:text-xs uppercase font-bold text-orange-500 tracking-widest">
          {t('progress.badge')}
        </div>
      ...
      {/* Desktop Dashboard Grid (Left: Activity & Stats, Right: Tech Breakdown & Badges) */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          >
            <motion.div variants={cardVariant} className="p-4 sm:p-5 rounded-2xl bg-[var(--bg-card)] border border-[var(--border-subtle)] text-center shadow-md">
              <div className="text-3xl sm:text-4xl lg:text-5xl font-serif-italic text-orange-400 font-light">{progress.xp}</div>
              <div className="text-[9px] sm:text-xs font-bold uppercase tracking-widest text-[var(--text-muted)] mt-1">{t('home.totalXp')}</div>
            </motion.div>

            <motion.div variants={cardVariant} className="p-4 sm:p-5 rounded-2xl bg-[var(--bg-card)] border border-[var(--border-subtle)] text-center shadow-md">
              <div className="text-3xl sm:text-4xl lg:text-5xl font-serif-italic text-[var(--text-primary)] font-light">{progress.streak}d</div>
              <div className="text-[9px] sm:text-xs font-bold uppercase tracking-widest text-[var(--text-muted)] mt-1">{t('home.streakStat')}</div>
            </motion.div>

            <motion.div variants={cardVariant} className="p-4 sm:p-5 rounded-2xl bg-[var(--bg-card)] border border-[var(--border-subtle)] text-center shadow-md">
              <div className="text-3xl sm:text-4xl lg:text-5xl font-serif-italic text-[var(--text-primary)] font-light">{totalCompletedLessons}</div>
              <div className="text-[9px] sm:text-xs font-bold uppercase tracking-widest text-[var(--text-muted)] mt-1">{t('home.lessonsDone')}</div>
            </motion.div>
          </motion.div>
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-orange-500/20 text-orange-400 border border-orange-500/30 flex items-center justify-center">
                  <Calendar className="w-4 h-4 sm:w-5 sm:h-5" />
                </div>
                <div>
                  <div className="text-[10px] sm:text-xs uppercase font-bold text-[var(--text-muted)] tracking-widest">
              ...
              <div className="px-3 py-1.5 rounded-xl bg-orange-500/10 border border-orange-500/30 text-orange-400 text-xs sm:text-sm font-black flex items-center gap-1">
                <Zap className="w-3.5 h-3.5 fill-orange-400" />
            ...
            <div className="w-full h-48 sm:h-56 pt-2">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={last7DaysData} margin={{ top: 12, right: 6, left: -20, bottom: 0 }}>
                  <XAxis
                    dataKey="day"
                    stroke="#71717A"
                    fontSize={11}
            ...
            {/* Resumo da Semana */}
            <div className="grid grid-cols-2 gap-3 pt-2 border-t border-[var(--border-subtle)] text-xs sm:text-sm">
              <div className="p-3 rounded-xl bg-[var(--bg-surface)] border border-[var(--border-subtle)] flex items-center gap-2.5">
                <TrendingUp className="w-4 h-4 sm:w-5 sm:h-5 text-orange-400 shrink-0" />
                <div>
                  <span className="text-[10px] sm:text-xs text-[var(--text-muted)] uppercase tracking-wider block font-bold">{t('progress.dailyAvg')}</span>
              ...
              <div className="p-3 rounded-xl bg-[var(--bg-surface)] border border-[var(--border-subtle)] flex items-center gap-2.5">
                ...
                <div>
                  <span className="text-[10px] sm:text-xs text-[var(--text-muted)] uppercase tracking-wider block font-bold">{t('progress.bestDay')}</span>
          ...
          <motion.div variants={fadeInUp} initial="initial" animate="animate" className="p-4 sm:p-5 rounded-2xl bg-[var(--bg-card)] border border-[var(--border-subtle)] shadow-sm">
            <div className="text-[10px] sm:text-xs font-bold uppercase tracking-widest text-[var(--text-muted)] mb-2">
              {t('progress.dailyInsightBadge')}
            </div>
            <div className="text-xs sm:text-sm italic text-[var(--text-muted)] font-serif leading-relaxed">
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <motion.div variants={fadeInUp} initial="initial" animate="animate" className="p-5 sm:p-6 rounded-2xl bg-[var(--bg-card)] border border-[var(--border-subtle)] space-y-4 shadow-md">
            <div className="flex items-center gap-2 text-xs sm:text-sm font-bold text-[var(--text-primary)] uppercase tracking-wider">
              ...
              {TECHNOLOGIES.map(tech => {
                ...
                return (
                  <div key={tech.id} className="space-y-1.5">
                    <div className="flex justify-between text-xs sm:text-sm text-[var(--text-primary)] font-medium">
                      <span className="flex items-center gap-2">
                        <span className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: tech.color }} />
                        <span className="font-bold">{tech.name}</span>
                      </span>
                      <span className="text-[var(--text-muted)] text-[11px] sm:text-xs">{completedCount} {t('home.lessonsCount')} ({pct}%)</span>
                    </div>
          ...
          {/* Conquistas / Badges */}
          <motion.div variants={fadeInUp} initial="initial" animate="animate" className="space-y-3.5">
            <div className="flex items-center justify-between">
